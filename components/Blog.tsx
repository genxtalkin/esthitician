...
// Detect if content is HTML (rich editor) or plain markdown (legacy)
const isHtmlContent = (text: string) => /<[a-z][\s\S]*>/i.test(text);

// Safe HTML renderer using DOMPurify
const SafeHtml: React.FC<{ html: string }> = ({ html }) => {
  const clean = DOMPurify.sanitize(html, {
    ALLOWED_TAGS: ['p','h1','h2','h3','h4','h5','h6','br','strong','em','u','s','a','ul','ol','li',
      'blockquote','code','pre','img','hr','span','div','figure','figcaption','video'],
    ALLOWED_ATTR: ['href','src','alt','title','class','style','controls','width','height',
...
const Blog: React.FC<BlogProps> = ({ meghanMode }) => {
  ...
  const [newPost, setNewPost] = useState({ title: '', category: 'Science', excerpt: '', content: '', imageUrl: '' });
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      if (supabase) {
        ...
        if (data && !error && data.length > 0) { setPosts(data); return; }
      }
      const cached = localStorage.getItem('meghan_blog_posts');
      setPosts(cached ? JSON.parse(cached) : defaultPosts);
    };
    fetchPosts();
  }, []);
  ...
  const handleAddPost = async (e: React.FormEvent) => {
    ...
    if (supabase) {
      ...
      if (error) { setSaveError(`Database error: ${error.message}`); return; }
    } else {
      try {
        localStorage.setItem('meghan_blog_posts', JSON.stringify([post, ...posts]));
      } catch {
        setSaveError("Storage limit reached. Try smaller images or use URLs instead.");
        return;
      }
    }
    setPosts([post, ...posts]);
  ...
  const handleHeroFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) { setHeroFileError(`File too large (max ${isVideo ? '50MB' : '4MB'}).`); return; }
    setHeroFileError('');
    ...
    if (supabase) {
      ...
      if (error) { setHeroFileError(`Upload failed: ${error.message}`); setIsUploadingHero(false); return; }
      const { data: { publicUrl } } = supabase.storage.from('blog-media').getPublicUrl(name);
      setNewPost(prev => ({ ...prev, imageUrl: publicUrl }));
    } else {
      const reader = new FileReader();
      reader.onloadend = () => { setNewPost(prev => ({ ...prev, imageUrl: reader.result as string })); setIsUploadingHero(false); };
      reader.onerror = () => { setHeroFileError("Failed to read file."); setIsUploadingHero(false); };
  ...
  const confirmDelete = async () => {
    ...
    if (supabase) {
      ...
      localStorage.setItem('meghan_blog_posts', JSON.stringify(updated));
    }
    ...
    <div className="flex-1 overflow-y-auto bg-[#faf9f6] p-4 md:p-10">
      <div className="max-w-4xl mx-auto">
        ...
        <header className="mb-12 text-center relative">
          ...
          {meghanMode && (
            <button
              onClick={() => { setIsAdding(!isAdding); setNewPost({ title: '', category: 'Science', excerpt: '', content: '', imageUrl: '' }); }}
        ...
        {/* ── New post form ── */}
        {isAdding && meghanMode && (
          <div className="mb-16 bg-white p-8 rounded-3xl border border-stone-200 shadow-xl">
            <h3 className="text-xl font-serif font-bold text-stone-900 mb-6">New Blog Entry</h3>
            {saveError && (
              <div className="mb-6 p-4 bg-red-50 border border-red-100 rounded-xl text-red-800 text-sm">{saveError}</div>
            )}
            <form onSubmit={handleAddPost} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-stone-50 border border-stone-100 p-3 rounded-xl focus:ring-2 focus:ring-rose-800/10 focus:border-rose-800 outline-none transition-all font-serif italic"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-stone-400 mb-2">Hero Image (Optional)</label>
                  <div className="flex gap-2">
                    <input
                      value={newPost.imageUrl}
                      onChange={e => setNewPost({ ...newPost, imageUrl: e.target.value })}
                      className="flex-1 bg-stone-50 border border-stone-100 p-3 rounded-xl focus:ring-2 focus:ring-rose-800/10 focus:border-rose-800 outline-none transition-all font-serif italic"
                      placeholder="https://... or upload →"
                    />
                    <label className={`cursor-pointer bg-stone-100 border border-stone-200 px-4 rounded-xl text-[10px] font-bold uppercase tracking-widest text-stone-500 flex items-center justify-center transition-colors ${isUploadingHero ? 'opacity-50 cursor-not-allowed' : 'hover:bg-stone-200'}`}>
                      {isUploadingHero ? '...' : 'Upload'}
                  ...
                  {newPost.imageUrl && !isUploadingHero && <p className="text-xs text-emerald-600 font-bold mt-1">✓ Hero media attached</p>}
                </div>
              </div>

              {/* Rich text editor */}
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-stone-400 mb-2">Content</label>
                <RichTextEditor
                  value={newPost.content}
                  onChange={html => setNewPost({ ...newPost, content: html })}
                  placeholder="Write your post here — use the toolbar for fonts, headings, images, links, and more..."
                />
              </div>
            ...
            <article key={post.id} className="group relative">
              ...
              {meghanMode && (
                <button
                  ...
                  className="absolute -top-3 -right-3 flex items-center gap-1.5 px-3 py-1.5 bg-white border border-rose-200 rounded-full text-rose-700 text-[10px] font-bold uppercase tracking-widest hover:bg-rose-50 hover:border-rose-400 shadow-sm transition-all z-10"
                  title="Delete post"
                >
              ...
              <div className="flex items-center gap-4 mb-3">
                <span className="text-[10px] font-bold uppercase tracking-widest text-rose-800 bg-rose-50 px-2 py-1 rounded">{post.category}</span>
              ...
              {post.imageUrl && (
                <div className="mb-6 rounded-2xl overflow-hidden border border-stone-200 shadow-sm">
                  {post.imageUrl.startsWith('data:video/') || post.imageUrl.match(/\.(mp4|webm|ogg|mov)$/i)
                    ...
                    : <img src={post.imageUrl} alt={post.title} className="w-full h-auto object-cover max-h-[400px]" referrerPolicy="no-referrer" />
                  }
                </div>
              )}
              ...
              {/* Render HTML (new posts) or Markdown (legacy posts) */}
                      ...
                      components={{
                        img: ({ src, alt, ...props }) => {
                          if (src?.startsWith('data:video/') || src?.match(/\.(mp4|webm|ogg|mov)$/i)) {
                            return <video src={src} controls className="rounded-xl shadow-sm my-4 max-h-[400px] w-full" />;
                          }
                          return <img src={src} alt={alt} {...props} className="rounded-xl shadow-sm my-4" referrerPolicy="no-referrer" />;
                        },
                        p: ({ ...props }) => <p className="mb-4" {...props} />,
                        a: ({ ...props }) => <a className="text-rose-800 underline hover:text-rose-900" {...props} />
                      }}
                    >
                      {post.content}
                    </Markdown>
      ...
      {postToDelete !== null && (
        <div className="fixed inset-0 bg-stone-900/50 flex items-center justify-center z-[100] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl text-center">
            ...
            <p className="text-stone-500 text-sm mb-8 leading-relaxed">This action cannot be undone.</p>
            <div className="flex justify-center gap-3">
              <button onClick={() => setPostToDelete(null)} className="px-6 py-3 text-stone-500 text-[10px] font-bold uppercase tracking-widest bg-stone-100 rounded-xl hover:bg-stone-200 transition-colors">
                Cancel
              </button>
              <button onClick={confirmDelete} className="px-6 py-3 bg-rose-800 text-white text-[10px] font-bold uppercase tracking-widest rounded-xl hover:bg-rose-900 shadow-md transition-colors">
                Delete
              </button>
            </div>
          </div>
